...
export function buscarTrilha(tecnologia) {
    try {
        ...
        const encontradas = dados.trilhas.filter((t) => {
            const emTecnologias = t.tecnologias.some((tec) => tec.toLowerCase().includes(termo));
...
const DESAFIOS = {
    ...
    avançado: [
        ...
        {
            nivel: "avançado",
            enunciado: "Implemente o algoritmo de compressão Run-Length Encoding (RLE).",
...
export function gerarDesafio(nivel) {
    const nivelNorm = nivel
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase();
    const chave = nivelNorm === "intermediario"
        ? "intermediário"
        : nivelNorm === "avancado"
            ? "avançado"
            : "iniciante";
...
// ─────────────────────────────────────────────────────────────────────────────
export function gerarCertificado(nome, trilha) {
    if (!nome || !trilha) {
        return {
            content: [
                {
    ...
    try {
        const dados = carregarTrilhas();
        const encontrada = dados.trilhas.find((t) => t.titulo.toLowerCase().includes(trilha.toLowerCase()) ||
    ...
    const codigo = "DIO-" +
        ...
        "-" +
        Math.random().toString(36).substring(2, 6).toUpperCase();
    const certificado = { nome, trilha, data, codigo, carga_horaria };
    return {
...
// ─────────────────────────────────────────────────────────────────────────────
export function listarTrilhas(categoria) {
    try {
        const dados = carregarTrilhas();
        const lista = categoria
            ? dados.trilhas.filter((t) => t.categoria.toLowerCase().includes(categoria.toLowerCase()))
            : dados.trilhas;
